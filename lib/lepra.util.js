module.exports = function(self) {
    ...
    utilInit.prototype.processHTMLPosts = function(data, postArray, type, screenBiggest){
        ...
        while(res !== null){
            ...
            img = '';
            if (resImg !== null) {
                img = 'http://src.sencha.io/' + screenBiggest + '/' + screenBiggest + '/'+encodeURIComponent(resImg[1]);

                body = body.replace(resImg[1], 'http://src.sencha.io/'+screenBiggest+'/'+encodeURIComponent(resImg[1]));
                // convert all image URIs to compressed ones
                resImg = imgReg.exec(body);
                while(resImg !== null){
                    body = body.replace(res[1], 'http://src.sencha.io/'+screenBiggest+'/'+encodeURIComponent(resImg[1]));
    ...
    utilInit.prototype.processJSONPosts = function(posts, postArray, screenBiggest){
        ...
        for(i in posts){
            ...
            if (res !== null) {
                ...
                img = 'http://src.sencha.io/' + screenBiggest +  '/' + screenBiggest + '/'+encodeURIComponent(res[1]);

                posts[i].body = posts[i].body.replace(res[1], 'http://src.sencha.io/'+screenBiggest+'/'+encodeURIComponent(res[1]));
                // convert all image URIs to compressed ones
                res = imgReg.exec(posts[i].body);
                while(res !== null){
